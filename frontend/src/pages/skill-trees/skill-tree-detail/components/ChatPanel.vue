...
async function loadOrCreateChat() {
  ...
  if (conversations.value.length > 0) await switchChat(conversations.value[0]._id)
  else await newChat()
}
...
async function sendMessage() {
  ...
  if (!currentChat.value.title) currentChat.value.title = content.slice(0, 50)
  ...
  <div class="chat-panel">
    <div class="chat-header">
      ...
      <div class="chat-header__actions">
        <div class="chat-history-wrap">
          <button class="chat-btn" @click="showHistory = !showHistory">{{ t('chat.history') }} ▾</button>
          <div v-if="showHistory" class="chat-history-dropdown">
            <div
              v-for="conversation in conversations"
              :key="conversation._id"
              class="chat-history-item"
              :class="{ active: currentChat?._id === conversation._id }"
              @click="switchChat(conversation._id)"
            >
              <div class="chat-history-item__title">{{ conversation.title || t('chat.newConversation') }}</div>
              <div v-if="conversation.preview" class="chat-history-item__preview">{{ conversation.preview }}</div>
            </div>
            <div v-if="conversations.length === 0" class="chat-history-empty">{{ t('chat.emptyHistory') }}</div>
          </div>
        </div>
        <button class="chat-btn chat-btn--primary" @click="newChat">+ {{ t('chat.newChat') }}</button>
      </div>
    </div>

    <div v-if="focusedNodeId" class="chat-node-tag">
      <span class="chat-node-tag__dot"></span>
      <span>{{ t('chat.focusedNode') }}</span>
    </div>

    <div class="chat-messages">
      <div v-if="!currentChat" class="chat-empty">{{ t('chat.loading') }}</div>
      <template v-else>
        <div v-if="currentChat.messages.length === 0 && !isStreaming" class="chat-empty">{{ t('chat.empty') }}</div>
        <div
    ...
    <div class="chat-input-area">
      <textarea
        ...
        @keydown="onKeydown"
      ></textarea>
      <button class="chat-send-btn" :disabled="isStreaming || !inputText.trim()" @click="sendMessage">
...
<style scoped>
.chat-panel { width: 360px; flex-shrink: 0; display: flex; flex-direction: column; border-left: 1px solid var(--border-color); background-color: var(--bg-card); height: 100%; overflow: hidden; }
.chat-header { flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-bottom: 1px solid var(--border-color); }
...
.chat-btn { padding: 4px 10px; border-radius: 6px; font-size: 12px; background-color: var(--bg-input); color: var(--text-secondary); border: 1px solid var(--border-color); cursor: pointer; transition: opacity 0.15s; }
.chat-btn:hover { opacity: 0.75; }
.chat-btn--primary { background-color: #7c3aed; color: #fff; border-color: #7c3aed; }
.chat-history-wrap { position: relative; }
.chat-history-dropdown { position: absolute; right: 0; top: calc(100% + 4px); width: 220px; background-color: var(--bg-card); border: 1px solid var(--border-color); border-radius: 8px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.15); z-index: 50; max-height: 240px; overflow-y: auto; }
.chat-history-item { padding: 8px 12px; cursor: pointer; border-bottom: 1px solid var(--border-color); transition: background 0.15s; }
...
.chat-history-item__title { font-size: 13px; color: var(--text-primary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.chat-history-item__preview { font-size: 11px; color: var(--text-muted); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-top: 2px; }
.chat-history-empty { padding: 12px; font-size: 12px; color: var(--text-muted); text-align: center; }
.chat-node-tag { flex-shrink: 0; display: flex; align-items: center; gap: 6px; padding: 5px 14px; background-color: rgba(124, 58, 237, 0.08); border-bottom: 1px solid var(--border-color); font-size: 11px; color: #7c3aed; }
.chat-node-tag__dot { width: 6px; height: 6px; border-radius: 50%; background-color: #7c3aed; flex-shrink: 0; }
.chat-messages { flex: 1; overflow-y: auto; padding: 14px; display: flex; flex-direction: column; gap: 10px; }
.chat-empty { text-align: center; color: var(--text-muted); font-size: 13px; margin-top: 40px; }
...
.chat-textarea { width: 100%; resize: none; padding: 8px 10px; border-radius: 8px; border: 1px solid var(--border-color); background-color: var(--bg-input); color: var(--text-primary); font-size: 13px; line-height: 1.5; outline: none; transition: border-color 0.15s; font-family: inherit; box-sizing: border-box; }
.chat-textarea:focus { border-color: #7c3aed; }
.chat-textarea:disabled { opacity: 0.6; }
.chat-send-btn { align-self: flex-end; padding: 6px 18px; border-radius: 8px; font-size: 13px; font-weight: 500; background-color: #7c3aed; color: #fff; border: none; cursor: pointer; transition: opacity 0.15s; }
